...
class Solver():
    ...
    def __init__(self, img):
        ...
        self.mask = np.zeros_like(self.img) # h * w
        ...
        self.result = np.zeros_like(self.img)
    ...
    def graphCut(self):
        ...
        for i in nodes:
            if self.g.get_segment(i) == 1:
                self.mask[i // self.w, i % self.w] = (255, 255, 255)

        np.copyto(self.result, self.img, where = self.mask)
        return self.result
