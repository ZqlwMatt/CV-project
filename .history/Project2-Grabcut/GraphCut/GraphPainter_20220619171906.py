...
class Solver():
    ...
    def __init__(self, img):
        ...
        self.fore_seeds = []
        self.back_seeds = []
        self.graph = (-1) * np.ones(img.shape[:2])  # draft
        self.graph.reshape((self.h, self.w))
    ...
    def n_link(self):
        for (y, x), idx in np.ndenumerate(self.graph):
            ...
            k = y * self.w + x
            w = np.exp(- np.sum(np.power(self.img[y, x+1] - self.img[y, x], 2)) / 2)
            self.add_edge(k, k+1, w, w)
            w = np.exp(- np.sum(np.power(self.img[y+1, x] - self.img[y, x], 2)) / 2)
    ...
    def graphCut(self):
        ...
        for i in nodes:
            if self.g.get_segment(i) == 1:
                self.mask[i / self.w, i % self.w] = 255

        self.img.copyTo(self.result, self.mask)
        return self.result
