...
class GMM:
    ...
    def addSample(self, ci, color):
        ...
        self.prod[ci] += np.dot(np.transpose(color), color)  # (n, ) array 进行 transpose 无效.
        self.count[ci] += 1
        self.total_count += 1
...
class Solver:
        ...
        for y in range(self.h):
            for x in range(self.w):
                if (y >= ly) & (y <= ry) & (x >= lx) & (x <= rx):
                    ...
                    self.graph[y, x] = 1 # mark foreground
                else:
                    self.backSamples.append(self.img[y, x])
                    self.graph[y, x] = 0 # mark background
    ...
    def calcBeta(self):
        """
        calculate the beta value according to picture contrast.
        """
        beta = 0.
    ...
    def assignComponents(self):
        """
        assign components for every pixel.
        """
        for (y, x), idx in np.ndenumerate(self.graph):
            if idx == 1:
