...
class GMM:
    ...
    def Pr(self, ci, color):
        ...
        if self.weight[ci] > 0:
            ...
            res = np.exp(-0.5 * mult[0][0]) / np.sqrt(self.cov_det[ci]) / np.sqrt(2 * np.pi)
        return res
    ...
    def addSample(self, ci, pixel):
        # w = pixel.copy().astype(np.float64)
        self.sum[ci] += pixel
        pixel = pixel.reshape(-1, 1)
        self.prod[ci] += np.dot(pixel, np.transpose(pixel))
        self.count[ci] += 1
        self.total_count += 1
...
class Solver:
    ...
    def grabCut(self, p1, p2, iternum = 1):
        self.initGMM(p1, p2)
        ...
        for (y, x), idx in np.ndenumerate(self.graph):
            if idx == 1:
                self.mask[y+1, x+1] = (True, True, True)
