...
class GMM():
    ...
    def Pr(self, color):
        res = 0
        for ci in range(k):
            res += self.weigths[ci] * self.Pr(ci, color)
        return res
...
class Solver():
    ...
    def initGMM(self, p1, p2):
        ...
        fore_res = KMeans_model.fit(self.foreSamples)
        back_res = KMeans_model.fit(self.backSamples)
    ...
    def n_link(self, g, beta = 1, gamma = 1):
        ...
        for (y, x), idx in np.ndenumerate(self.graph):
            ...
            if x > 0:
                ...
                w = gamma * np.exp(- beta * np.power(diff, 2))
                g.add_edge(k, k-1, w, w)
            if (x > 0) & (y > 0):
                diff = self.img[y, x] - self.img[y-1, x-1]
                w = gammaDivSqrt2 * np.exp(- beta * np.power(diff, 2))
    ...
    def t_link(self, g):
        for (y, x), idx in np.ndenumerate(self.graph):
            k = y * self.w + x
            color = self.img[y, x]
            g.add_tedge(k, -np.log(self.foreGMM.Pr(color)), -np.log(self.backGMM.Pr(color)))
