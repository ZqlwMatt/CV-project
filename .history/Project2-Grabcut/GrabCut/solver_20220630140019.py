...
class Solver:
    ...
    def initGMM(self, rect, iternum = 200):
        ...
        for y in range(self.h):
            for x in range(self.w):
                if self.graph[y, x] > 1: continue
                if (y >= ly) & (y <= ry) & (x >= lx) & (x <= rx):
                    self.foreSamples.append(self.img[y, x])
                    self.graph[y, x] = 1 # mark prob foreground
                else:
                    self.backSamples.append(self.img[y, x])
                    self.graph[y, x] = 0 # mark prob background
    ...
    def segmentGraph(self):
        ...
        for (y, x), idx in np.ndenumerate(self.graph):
            ...
            if x > 0:
                ...
                w = gamma * np.exp(- beta * (diff*diff).sum())
                g.add_edge(k, k-1, w, w)
            if (x > 0) & (y > 0):
                diff = color - self.img[y-1, x-1]
                w = gammaDivSqrt2 * np.exp(- beta * (diff*diff).sum())
    ...
    def grabCut(self, rect, iternum = 2):
        self.initGMM(rect)
        self.beta = self.calcBeta()
        print("beta = " + str(self.beta))

        for _ in range(iternum):
            print(f'The {_}th/nd iteration begins.')
