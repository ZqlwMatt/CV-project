...
class Solver():
    ...
    def __init__(self, img):
        ...
        self.g = maxflow.Graph[float](self.size, (4 * self.size - 3 * self.h - 3 * self.w + 2))
    ...
    def n_link(self, beta, gamma):
        ...
        for (y, x), idx in np.enumerate(self.graph):
            ...
            if (x > 0) & (y > 0):
                ...
                self.g.add_edge(k, k-1-self.w, gammaDivSqrt2 * np.exp(- beta * np.power(diff, 2)))
            if y > 0:
                diff = self.img[y, x] - self.img[y-1, x]
                self.g.add_edge(k, k-self.w, gamma * np.exp(- beta * np.power(diff, 2)))
            if (y > 0) & (x < self.w-1):
                diff = self.img[y, x] - self.img[y-1, x+1]
                self.g.add_edge(k, k-self.w+1, gammaDivSqrt2 * np.exp(- beta * np.power(diff, 2)))
