...
class Solver():
    ...
    def _grabCut(self, p1, p2):
        mask = np.zeros_like(self.img[:2], np.uint8)
        lx, ly = min(p1[0], p2[0]), min(p1[1], p2[1])
        rx, ry = max(p1[0], p2[0]), max(p1[1], p2[1])
        ...
        cv.grabCut(self.img, mask=mask, rect=rect, bgdModel=bgdModel, fgdModel=fgdModel, iterCount=5, mode=cv.GC_INIT_WITH_MASK)
