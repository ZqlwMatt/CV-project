...
class GMM:
    ...
    def Pr(self, ci, color):
        ...
        if self.weight[ci] > 0.:
            ...
            res = (1. / np.sqrt(self.cov_det[ci]) * np.exp(-0.5 * mult))[0][0]
            # 常数项直接丢掉
        return res
...
class Solver:
    ...
    def initGMM(self, p1, p2, iternum = 100):
        ...
        n_iter_: 迭代运行的次数。
        """
        KMeans_model_fore = KMeans(n_clusters = k, n_units=3, max_iter = iternum)
        fore_res = KMeans_model_fore.fit(self.foreSamples)
        KMeans_model_back = KMeans(n_clusters = k, n_units=3, max_iter = iternum)
