...
class GMM:
    ...
    def Pr(self, ci, color):
        ...
        if self.weight[ci] > 0:
            ...
            mult = np.dot(diff, np.dot(self.cov_inv[ci], np.transpose(diff)))
    ...
    def addSample(self, ci, w):
        self.sum[ci] += w
        self.prod[ci] += np.dot(np.transpose(w), w)
        self.count[ci] += 1
        self.total_count += 1
...
class Solver:
    def __init__(self, img):
        ...
        self.components = np.zeros((self.h, self.w), dtype=np.int64) # temporary components
    ...
    def initGMM(self, p1, p2, iternum = 100):
        ...
        cluster_centers_: 聚类中心的坐标。（如果算法还未完全收敛就停止，则将与labels_不一致）
        labels_: 每个点的标签。
        inertia_: 样本到聚类中心的平方和。
        n_iter_: 迭代运行的次数。
        """
        KMeans_model_fore = KMeans(n_clusters = k, n_init=1, max_iter = iternum)
        fore_res = KMeans_model_fore.fit(self.foreSamples)
        KMeans_model_back = KMeans(n_clusters = k, n_init=1, max_iter = iternum)
    ...
    def assignComponents(self):
        for (y, x), idx in np.ndenumerate(self.graph):
            if idx == 1:
                self.components[y, x] = self.foreGMM.whichComponent()

    def learnGMM(self):
        self.foreGMM.clear()
        self.backGMM.clear()

        for (y, x), idx in np.ndenumerate(self.components):
            color = self.img[y, x].copy()
            if idx == 1:
                self.foreGMM.addSample(idx, color)
            else:
                self.backGMM.addSample(idx, color)
    ...
    def grabCut(self, p1, p2, iternum = 3):
        self.initGMM(p1, p2)
