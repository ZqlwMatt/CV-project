...
class Solver():
    ...
    def t_link(self, g):
        ...
        for (y, x), idx in np.ndenumerate(self.graph):
            k = y * self.w + x
            if idx == 0:
                g.add_tedge(k, self.inf, 0)
            elif idx == 1:
                g.add_tedge(k, 0, self.inf)
            else:
                w = self.calc_gray(y, x)
                g.add_tedge(k, self.alpha * w[0], self.alpha * w[1])
    ...
    def segmentGraph(self, g):
        ...
        self.t_link(g)
        for i in nodes:
            self.graph[i // self.w, i % self.w] = g.get_segment(u)
            if seg.get_segment(i) == 1:
