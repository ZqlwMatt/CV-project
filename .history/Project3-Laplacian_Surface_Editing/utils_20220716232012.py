"""
This lib is used for
(1) transformation between plyfile and numpy, 
(2) graph construction and operation.
"""
import numpy as np
import igraph as ig
...
# python-plyfile guideline: https://github.com/dranjan/python-plyfile
...
def get_border(g, ROI_vertices):
    ...
    n = len(ROI_vertices)
    for i in range(n):
        segment = g.get_shortest_paths(ROI_vertices[i], ROI_vertices[(i+1)%n], output='vpath')[0] # 返回 list 包含(所有)可行路径
        border += segment[0:-1]
    return border
