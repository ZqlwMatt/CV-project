...
if __name__ == "__main__":
    ...
    for i in range(n):
        vertex[i] = tuple(vertex[i])[:3]
        
    out_vertex = PlyElement.describe(vertex, 'vertex')
    out_face = PlyElement.describe(face, 'face')
    PlyData([out_vertex, out_face], text=True).write(PATH+'test.ply')