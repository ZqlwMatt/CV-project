...
if __name__ == "__main__":
    ...
    sub_V = np.vstack(tuple(vertex[subToAbs[i]] for i in range(n))) # 按照 igraph subgraph 的顺序建立点矩阵
    ...
    for i in range(n):
        ...
        Ti = scipy.sparse.linalg.pinv(Ai)
        ...
        T_delta = np.array([
                 si*Delta_ix - hi[2]*Delta_iy + hi[1]*Delta_iz, # x 的T变换方程
              hi[2]*Delta_ix + si*Delta_iy    - hi[1]*Delta_iz, # y 的T变换方程
            - hi[1]*Delta_ix + hi[0]*Delta_iy +     si*Delta_iz # z 的T变换方程
        ])
