...
ply_data = PlyData.read('/Users/zqlwmatt/project/CV-project/Laplacian-Surface-Editing/meshs/bun_zipper.ply')
# ply_data = PlyData.read('/Users/zqlwmatt/project/CV-project/Laplacian-Surface-Editing/meshs/dragon_vrip.ply')
...
for i in range(len(ROI_vertices)):
    b = np.append(b, 100 * V[total_to_sub[ROI_vertices[i]]])
    ROI_vertex_index_sub = total_to_sub[ROI_vertices[i]]
    L_prime[3 * (morph_num + len(handle_vertices)) + i * 3 + 0, ROI_vertex_index_sub + 0 * morph_num] = 100
    L_prime[3 * (morph_num + len(handle_vertices)) + i * 3 + 1, ROI_vertex_index_sub + 1 * morph_num] = 100
    L_prime[3 * (morph_num + len(handle_vertices)) + i * 3 + 2, ROI_vertex_index_sub + 2 * morph_num] = 100

# print(b)
b = np.hstack([np.zeros(3 * morph_num), b]) # b 初始值全为 0
...
ply_data.write('/Users/zqlwmatt/project/CV-project/Laplacian-Surface-Editing/edit_meshs/bun_zipper_edited.ply')
# ply_data.write('/Users/zqlwmatt/project/CV-project/Laplacian-Surface-Editing/edit_meshs/dragon_vrip_edited.ply')
